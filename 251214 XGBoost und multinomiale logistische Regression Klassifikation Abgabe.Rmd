...
suppressPackageStartupMessages({
  ...
  library(reshape2)
  library(report)
  library(nnet)           # für das multinomiale Modell
  library(glmnet)         # für das multinomiale Modell 
  library(mltools)
  
  
  # SHAP-bezogene Pakete
  library(shapr)          # SHAP-Werte für Erklärbarkeit
...
```
Konvertierung der Daten und Entfernung aller Zeilen mit fehlender Zielvariable, Überprüfung fehlender Daten:
```{r data_cleaning}
# ---- 3. DATENBEREINIGUNG UND -AUFBEREITUNG ----
data_clean <- data_raw %>%
  # Entferne Zeilen mit fehlender Zielvariable
  filter(!is.na(.data[[target_var]])) %>%
  # Konvertiere SPSS Labels zu numerischen Werten (nur für nicht-character Variablen)
  mutate(across(where(is.labelled) & !where(is.character), ~ as.numeric(.x))) %>%
  # Für character-labelled Variablen: Entferne Labels und behalte als character
...
```
Definition und Überprüfung der betrachteten Prädiktorvariablen:
```{r basic_feature_selection}
# ---- 4. FEATURE-AUSWAHL MIT INCLUDE-LISTE ----

# Definiere ALLE gewünschten Features (ohne Zielvariable)
include_vars <- c(
...
cat("Gewünschte Features:", length(include_vars), "\n\n")

# PRÜFUNG 1: Welche Variablen existieren im Datensatz?
existing_vars <- intersect(include_vars, names(data_clean))
missing_vars <- setdiff(include_vars, names(data_clean))

if(length(missing_vars) > 0) {
  cat("\n⚠️ Fehlende Features (", length(missing_vars), "):\n", sep="")
  print(missing_vars)
  cat("\nDiese werden ignoriert.\n")
}

# PRÜFUNG 2: Sind alle numerisch?
non_numeric <- existing_vars[!sapply(data_clean[existing_vars], is.numeric)]
if(length(non_numeric) > 0) {
  cat("\n⚠️ Nicht-numerische Features (", length(non_numeric), "):\n", sep="")
  print(non_numeric)
...
p_target <- ggplot(data_basic, aes_string(x = target_var)) +
  ...
  labs(title = paste("Verteilung der Zielvariable:", target_var),
       x = "Likert-Skala (1-7)", y = "Häufigkeit") +
  theme_minimal() +
  scale_x_continuous(breaks = 1:7)

print(p_target)

# Einfache Imputation mit Median für Korrelationsanalyse
...
# Korrelationsanalyse für Top Features
...
# Top Features für Korrelationsmatrix
...
```
Vorbereiten der Daten für XGBoost - keine Imputation notwendig (Variablennamen können aufgrund von Codeübernahmen von Random Forest irreführend sein)
```{r imputing}
# ---- 6. DATEN FÜR XGBOOST VORBEREITEN ----
...
xgb_tuned <- train(
  x = X_train_matrix,
  y = as.factor(y_train_vector),  # Als Faktor für Klassifikation
  method = "xgbTree",
  trControl = ctrl,
  tuneGrid = xgb_grid,
...
cat("✅ XGBoost Modell erfolgreich trainiert.\n")
```
Anpassen der Labels für spätere Visualisierungen:
```{r}
# ==== LABELS FÜR VISUALISIERUNG ====
variable_labels <- c(
  "a1l1_333" = "Ökologisch: Smarte Energieversorgung",
  "a1l2_334" = "Ökologisch: Vielfältige Begrünung",
  "a1l3_335" = "Ökologisch: Zirkuläre Wirtschaft",
  "a1l4_336" = "Ökologisch: Emissionsarmes Quartier",
  "a1l5_337" = "Ökologisch: Versickerungsflächen",
  ...
  "a2l4_341" = "Sozial: Generationenübergreifend",
  "a2l5_342" = "Sozial: Mitbestimmung",
  "a3l1_343" = "Ökonomisch: Lokale Unternehmen",
  "a3l2_344" = "Ökonomisch: Internationale Unternehmen",
  "a3l3_345" = "Ökonomisch: Start-Ups & Innovation",
  "a3l4_346" = "Ökonomisch: Bildung",
  "a3l5_347" = "Ökonomisch: Verkehrsinfrastruktur",
  "a3l6_348" = "Ökonomisch: Kunst & Kultur"
)
...
Berechnung der Gütemaße des XGBoost-Modells: 
```{r}
# ----- Gütemaße -----
X_test_matrix <- as.matrix(test_basic[feature_cols])

# Vorhersage gibt Matrix mit Wahrscheinlichkeiten zurück
predict <- predict(xgb_model, X_test_matrix, reshape = TRUE)

# Extrahiere die Klasse mit höchster Wahrscheinlichkeit
predict_class <- max.col(predict)  # Gibt Spaltenindex (1-7) zurück
...
plot(test_basic[[target_var]], predict_class,
    xlab = "Tatsächliche Klassen",
    ylab = "Vorhergesagte Klassen",
    main = "XGBoost Klassifikation: Vorhersage vs. Realität")
abline(0, 1, col = "red", lwd = 2)

# Berechne MCC für XGBoost
...
# Macro-averaged F1-Score (Durchschnitt über alle Klassen)
f1_macro_xgb <- mean(f1_scores_xgb, na.rm = TRUE)
cat("Macro F1-Score des XGBoost Modells:", round(f1_macro_xgb, 4), "\n\n")
```


Berechnung der multinomialen logistischen Regression, inkl. Imputation der fehlenden Werte. Anschließende Berechnung der Gütemaße des Modells:
```{r}
...
# F1-Score für imputiertes multinomiales Modell
...
# Ausgabe für jede Klasse (relativ zu Referenzklasse 1)
...
pred_long <- pred_df %>%
  pivot_longer(cols = c(Actual, XGBoost, Multinomial),
               names_to = "Modell",
               values_to = "Klasse") %>%
  mutate(Modell = ifelse(Modell == "Actual", "Tatsächliche Werte", Modell),
         Modell = factor(Modell, levels = c("Tatsächliche Werte", "Multinomial", "XGBoost")))
...
p_hist <- ggplot(pred_long, aes(x = Klasse, fill = Modell)) +
  ...
  scale_x_continuous(breaks = 1:7) +
  scale_fill_manual(values = c("Tatsächliche Werte" = "gray50", 
                               "XGBoost" = "steelblue", 
                               "Multinomial" = "coral")) +
...
p_heatmap_signif <- ggplot(plot_data, aes(x = Klasse, y = Feature, fill = Beta)) +
  ...
  labs(title = "Multinomiale Regression: Beta-Koeffizienten (Top 10)",
       ...
       x = "Klasse im Verhältnis zu Referenzklasse 1", 
       y = "",
       fill = "Beta") +
  theme_minimal() +
...
cat("\n✅ Heatmap mit Beta + Signifikanz  erstellt!\n")
# Berechne minimalen p-Wert pro Feature über alle Klassen
...
# Sortiere nach niedrigstem p-Wert (höchste Signifikanz)
...
Berechnung des Shapley-Wertes für alle Klassen des XGBoost-Modells:
```{r}
# ==== ANGEPASSTE VISUALISIERUNG FÜR ALLE KLASSEN ====
...
# Konvertiere zu Matrix für XGBoost
...
# SHAP für alle Klassen berechnen
shap_all_classes <- list()
shap_viz_all <- list()

cat("Berechne SHAP-Werte für alle Klassen...\n")
...
for(class_i in 1:n_classes) {
  ...
  for(i in 1:n_samples) {
    ...
    shap_values_i <- explain(
      ...
      nsim = 20, # je höher, desto besser. Dauert aber lange zum Rechnen
...
# ==== GENAUIGKEITSCHECK ====
library (patchwork)
cat("\n=== SHAP Genauigkeitscheck für alle Klassen ===\n")
...
for(class_i in 1:n_classes) {
  ...
  # Tatsächliche Vorhersagen für diese Klasse
...
combined <- combined + plot_annotation(
  title = "SHAP Genauigkeit für alle Klassen",
...
cat("✅ Importance Matrix erfolgreich berechnet!\n")
cat("Dimensionen:", nrow(importance_matrix), "Features x", ncol(importance_matrix), "Klassen\n\n")
```

Berechnung und Visualisierung der wichtigsten Features für das XGBoost-Modell und die multinomiale logistische Regression:
```{r}
...
pheatmap(importance_for_plot,
         main = "Feature Importance Heatmap\n(Top 15 Features über alle Klassen)",
         cluster_rows = TRUE,
         cluster_cols = FALSE,
         scale = "row",
...
# Erstelle Long-Format für ggplot
...
for(feat_idx in 1:10) {
  ...
  cat("\nErstelle Dependence Plots für:", top_feature_label, "\n")
  ...
  # Erstelle Plots für alle Klassen
  dep_plots <- list()
...
cat("Diese Lösung arbeitet direkt mit den Original-SHAP-Matrizen.\n")

# ==== VISUALISIERUNG TOP 10 VARIABLEN MULTINOMINAL ====

# Erstelle Long-Format für gestapelten Plot
...
# Berechne Gesamt-Importance für Sortierung (wichtigste oben)
...
cat("\n✅ Alle Visualisierungen mit Labels erfolgreich erstellt!\n")
```
